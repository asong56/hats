...
import (
	...
	"github.com/redew/redew/internal/model"
)
...
// ClearReadItemsCache deletes read items older than olderThanDays and
// reclaims disk space. Bookmarked articles are untouched: bookmarks are
// stored as independent content snapshots in the bookmarks table, not as
// references into items, so clearing the item cache never loses a save.
// Returns the number of items deleted.
func (s *Store) ClearReadItemsCache(olderThanDays int) (int64, error) {
	if olderThanDays < 0 {
		olderThanDays = 0
	}
	cutoff := fmt.Sprintf("unixepoch('now', '-%d days')", olderThanDays)

	res, err := s.db.Exec(`
		DELETE FROM items
		WHERE unread = 0 AND created_at < ` + cutoff + `
	`)
	if err != nil {
		return 0, fmt.Errorf("clear read items: %w", err)
	}
	...
	if _, err := s.db.Exec(`VACUUM`); err != nil {
		return deleted, fmt.Errorf("vacuum after clearing cache: %w", err)
	}
